...
export class AxiosCreateRequest {
  ...
  async makeRequest<T>(axiosRequestConfig: RequestConfig) {
    try {
      ...
      const axiosResponse: AxiosResponse | undefined =
        await this.executeRequest(axiosRequestConfig);
  ...
  buildApiReturnWithExceptionData<T>(
  ...
  buildApiReturnWithSucessData<T>(
    axiosResponse: AxiosResponse<any, any>,
  ): ApiReturn<T> {
    return {
      return: axiosResponse?.data,
  ...
  applyResponseLog(axiosResponse: AxiosResponse<any, any>): void {
    ...
    this.responseLogger.log(
      `Headers - ${JSON.stringify(axiosResponse?.headers)}`,
    );
    this.responseLogger.log(
      `Status - ${JSON.stringify(axiosResponse?.status)}`,
    );
    this.responseLogger.log(
      `Status Text - ${JSON.stringify(axiosResponse?.statusText)}`,
    );
    this.responseLogger.log(
      `Config - ${JSON.stringify(axiosResponse?.config)}`,
    );
  }
